...
const Entity = function(readData, ignoredColumns, kebabCase) {
  ...
  if (!ignoredColumns) {
    ...
    this.ignoredColumns.map(colname => {
      colname.toLowerCase();
    });
  }
    ...
    for (var prop in this.rawData) {
      ...
      if (Object.prototype.hasOwnProperty.call(this.rawData, prop)) {
        if (prop.indexOf('.') > -1) {
          ...
          if (this.kebabCase) {
            subEntityName =
              subEntityName.charAt(0).toLowerCase() +
              subEntityName.substring(1);
            subEntityPropName =
              subEntityPropName.charAt(0).toLowerCase() +
              subEntityPropName.substring(1);
          }
...
const asModel = ({ dbRead, modelType, modelsName, excludedColumns }) => {
  if (isModel(new modelType())) {
    const digested = digest(dbRead, {
      excludedColumns: excludedColumns,
    });
    const models = [];
    let model;
    digested.forEach(data => {
      model = new modelType();
      model.mapProperties(data);
      models.push(model);
    });

    return models.length < 2 ? models[0] : models;
  }
  return null;
};

const isSimple = dbResponse => {
  if (typeof dbResponse === 'number' || typeof dbResponse === 'boolean') {
    return true;
  } else {
    return false;
  }
...
const digest = function(dbResponse /*odczyt z bazy*/, options) {
  ...
  try {
    const records =
      dbResponse.rowsAffected.length > 1
