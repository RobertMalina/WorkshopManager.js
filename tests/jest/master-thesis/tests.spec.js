...
describe('mocks related tests', () => {
  ...
  describe('OrderService.fetchAsPageContent', () => {
    ...
    test('getOrdersCount should return 5', async () => {
      ...
      expect.assertions(1);
      const result = await orderService.getOrdersCount(statusFilters);
    ...
    test('should return fixed test data', async () => {
      ...
      const result = await orderService.fetchAsPageContent({
        page: 0,
        itemsOnPage: 5,
        statusFilters,
      });
      expect(JSON.stringify(result).replace(/\s+/g, '')).toEqual(
        JSON.stringify(pagedOrders).replace(/\s+/g, ''),
      );
    });
  });
});
