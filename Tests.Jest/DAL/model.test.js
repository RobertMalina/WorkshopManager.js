const { 
  Model, 
  AppUser, 
  AppUserToAppRole } = require('../../DAL/DAL.index.js');

  describe('AppUser:Model inheritance test', () => {

    const user = new AppUser();

    describe.skip('AppUser should have:', () => {
      test('public set function', () => { expect(user.set).toBeTruthy()})
      test('public get function', () => { expect(user.get).toBeTruthy()})
      test('public canAccept function', () => { expect(user.canAccept).toBeTruthy()})
      test('Id property', () => { expect(user.get('Id')).toBeTruthy()})
      test('Username property', () => { expect(user.get('Username')).toBeTruthy()})
      test('PasswordHash property', () => { expect(user.get('PasswordHash')).toBeTruthy()})
      test('Roles property', () => { expect(user.get('Roles')).toBeTruthy()})
    });
  });
  ...
  describe('Model functionality test', () => {
    ...
    describe('form data mapping', () => {
      ...
      test('should be successfull, so', () => {
        ...
        expect(user.get('PasswordHash')).toEqual('hashhashhash');
        expect(user.get('Roles')).toEqual(expect.arrayContaining(['regular','mechanician']));
      });
      ...
      test('should be successfull as well (inaccurate letter cases)', () => {
        ...
        expect(user.get('Username')).toEqual('testuser');
        expect(user.get('PasswordHash')).toEqual('hashhashhash');
        expect(user.get('Roles')).toEqual(expect.arrayContaining(['regular','admin']));
      });
    })
