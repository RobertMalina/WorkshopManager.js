function DbConfigurator() {
    ...
    switch (options.target) {
      case 'dbname':
        if (dbDestinationFlag === '-test') {
          ...
          console.log(
            `process.env.DEV_DB_NAME___NEW set to ${process.env.DEV_DB_NAME___NEW}`,
          );
        } else {
          //apply changes to development db connection settings
          process.env.DEV_DB_NAME___NEW = options.value;
          console.log(
            `process.env.DEV_DB_NAME___NEW set to ${process.env.DEV_DB_NAME___NEW}`,
          );
      ...
      case 'servername':
        ...
        reloadConnectionSettings();
        console.log(
          `process.env.DEV_DB_NAME___NEW set to ${process.env.DEV_SQLSERVER_IP_ADDR___NEW}`,
        );
        break;
      default:
        break;
  ...
  const inspectConnectionSettings = (/*string: -dev || -test*/ dbType) => {
    switch (dbType) {
      case '-dev':
        reloadConnectionSettings();
        console.log(config.getDbSettings('-dev'));
        break;
      case '-test':
        reloadConnectionSettings();
        console.log(config.getDbSettings('-test'));
        break;
      default:
  ...
  this.handleCommand = cmd => {
    ...
    if (operation === 'config') {
      if (args.indexOf('config') !== -1 && args.length < 4) {
        ...
        let configSrc, settings;

        switch (confTarget) {
          case 'dbname':
          case 'servername':
            changeConnectionSettings(
              { target: args[2], value: args[3] },
              args[4],
            );
            break;
          case 'inspect':
