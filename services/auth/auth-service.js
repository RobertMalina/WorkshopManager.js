...
const {
  AuthProvider,
  PassportLocalAuthProvider,
  JWTAuthProvider,
} = require('./providers/providers.index');
...
const AuthService = function(
    ...
    */,
    ...
    if (config.target) {
      switch (config.provider) {
        ...
        default:
          console.error(
            `Authorization provider ${config.provider} is not registered in AuthService.setAuthentication method...`,
          );
          return false;
      }
  ...
  this.usersSystemApi = {
    ...
    getUser: function(
      /*id or username*/ identifier,
      withRoles = false,
    ) /*: Promise<AppUser?> */ {
      return new Promise((resolve, reject) => {
        const query = queryStore.get('selectAppUser', {
    ...
    createAppUserInstance: function(
      userData = { password, username, roles },
    ) /*: Promise<AppUser?> */ {
      return new Promise((resolve, reject) => {
    ...
    register: function(
      ...
      return new Promise((resolve, reject) => {
        this.createAppUserInstance(userData)
          ...
          .then(insertResult =>
            this.getUser(insertResult.model.get('Username')),
          )
    ...
    checkCredentials: function(username, password) {
      /*: Promise<{ user: AppUser, result: boolean, isError?: boolean}> */
