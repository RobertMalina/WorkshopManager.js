...
const AuthProvider = function(config) {
  ...
  const defaults = {
    ...
    onUserSystemSuccess: (res, user) => {
      console.warn(
        'User data is correct, but authentication state persisting is not implemented',
      );
      res.json({
        msg:
          'Authentication passed, but no persisted - server does not implement auth-persistence layer.',
        isAuthenticated: true,
        userName: user instanceof AppUser ? user.get('Username') : '???',
      });
    },
  };
  ...
  */
  if (!config) {
    console.error(
      'AuthProvider can not be instantiated without configuration object...',
    );
    return null;
  }
  ...
  if (hasNoSpecificProvider) {
    ...
    if (typeof config.authHandler !== 'function') {
      console.error(
        'obligatory function authHandler of AuthProvider is not provided...',
      );
      return null;
    }
    if (typeof config.onUserSystemSuccess !== 'function') {
      console.error(
        'obligatory function onUserSystemSuccess of AuthProvider is not provided...',
      );
      return null;
    }
  ...
  this.loginSuccessHandler =
    config.logInSuccessHandler || defaults.loginSuccessHandler;

  this.loginFailedHandler =
    config.loginFailedHandler || defaults.loginFailedHandler;

  this.activateOn = (server /*AppServer*/) => {
    //if(!server instanceof AppServer) { //TODO weryfikacja
    if (!server) {
      new Error('server object has invalid type or not provided...');
    }
